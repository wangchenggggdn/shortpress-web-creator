...
import { EditWebsite, Version, SectionType } from '@/types/editor';
...
const WebsitePreviewPage: React.FC<WebsitePreviewPageProps> = ({ params }) => {
    ...
    const handlePublish = async () => {
        ...
        try {
            ...
            // 收集共享 sections
            if (currentVersion.shareSections) {
            ...
            if (translationItems.length > 0) {
                try {
                    ...
                    const response = await WebsiteApi.translate(editWebsite.id, translationRequest);

                    if (response.code === 0 && response.data && response.data.length > 0) {
                        // 应用翻译结果
                        const updatedWebsite = TranslationHelper.applyTranslationResults(editWebsite, response.data);

                        // 保存翻译后的数据
                        await WebsiteApi.editModify(editWebsite.id, updatedWebsite);

                        toast.success('Translation completed');
                    }
                } catch (error) {
                    console.error('Translation error:', error);
                    toast.warning('Translation failed, publishing without translation');
                }
            }
        ...
        <div className="min-h-screen">
            ...
            {versionNumber !== lastVersionNumber && (
                <div className="bg-yellow-50 border-b border-yellow-100 p-4 text-center">
                    ...
                    <button
                        className="ml-2 text-indigo-600 hover:text-indigo-700 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <div className="flex justify-center py-8 px-4">
                <div className={`bg-white shadow-lg  w-[350px] h-[calc(100vh-200px)]`}>
                    ...
                    <div className="relative h-full">
                        <IframePage url={`${getWebsitePreviewUrl(website, currentPage || '')}`} />
                    </div>
                </div>
            </div>
        </div>
