import { WidgetType } from '@/types/editor';
import React from 'react';
import { IconMenuItemProps } from './types';

const IconMenuItem: React.FC<IconMenuItemProps> = ({ title, widget, onToggle }) => {
    const isScrollItem = widget?.type === WidgetType.SCROLL;
    return (
        <div className="mb-4 p-4 bg-white border border-gray-200 rounded-xl">
            <div className="flex items-center justify-between">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                    {isScrollItem ? (
                        <input type="checkbox" className="sr-only peer" checked={widget?.scroll ?? false} onChange={onToggle} />
                    ) : (
