    ...
    return (
        ...
        >
            <div className="flex items-center gap-2 flex-1">
                ...
                <span className={clsx('ml-2', (item.visible=== undefined?true: item.visible)? 'text-black-purple': 'text-gray-400')}>
                    {item.label}
                </span>
            </div>
            ...
            <Menu position="bottom-end" offset={4} withArrow>
                ...
                <Menu.Dropdown>
                    <Menu.Item onClick={() => onToggleVisibility(item.id)}>
                        {(item.visible=== undefined?true: item.visible) ? 'Hide in Menu' : 'Show in Menu'}
                    </Menu.Item>
                    <Menu.Item onClick={() => onDuplicate(item.id)}>
                        Duplicate
                    </Menu.Item>
                    <Menu.Item onClick={() => onRename(item)}>
                        Rename
                    </Menu.Item>
                    <Menu.Divider />
